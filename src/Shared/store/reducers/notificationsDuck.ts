 ...
 * @returns 
 */
export const restoreViewedNotificationsAction = (): ThunkAppAction => async (dispatch, _) => {
    dispatch({
        type: GET_NOTIFICATIONS_SUCCESS,
        payload: {
            viewedNotifications: await retrieveUnseenNotifications()
        }
    });
}
 ...
 */
const persistUnseenNotifications = async (
    viewedNotificationsInState: NotificationViewsDictionary
) => {
 ...
 */
const retrieveUnseenNotifications = async (): Promise<NotificationViewsDictionary> => {
    const storedNotifications = await AsyncStorage.getItem(VIEWED_NOTIFICATIONS);
